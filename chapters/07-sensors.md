...
**実験2：**
- やったこと：
startTime と endTime に関係するタイマー部分（elapsedTime 関数）で、max(0, ...) のセーフティネットを外してみた。

- 結果：
ボタンを押した瞬間のほんのわずかなズレのせいで、タイマーの表示が一瞬だけ「-01秒」とマイナス表示になってからカウントアップが始まった。

- わかったこと：
時間の計算はスマホの処理速度のズレで簡単にバグるので、max(0, ...) を使って「最低でも0秒からスタートする」とコードで縛るのが大事だと分かった。
...
CoreMotion（歩数）や CoreLocation（GPS）といったスマホのハードウェアを動かすときは、**「裏側で動くセンサーデータを、どう安全に画面（UI）に届けるか」** が一番の重要ポイント。
