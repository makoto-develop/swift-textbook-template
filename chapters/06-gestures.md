...
- 発表テーマ：SwiftUIで複数のジェスチャーを同時に扱う方法
- 発表箇所：第6章「ジェスチャー操作」
- 見せる内容：`.gesture()` と `.simultaneousGesture()` の違い、ドラッグ速度の実験
- 発表の流れ：
  1. テーマと選んだ理由を30秒で説明
  2. ドラッグ・拡大縮小・回転のコードを説明
  3. ジェスチャーが競合した実験結果を紹介
  4. `.simultaneousGesture()` に変更した結果を説明
  5. 実験から学んだことを30秒でまとめる
